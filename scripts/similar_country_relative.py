...
if __name__ == "__main__":
    ...
    similar_country_relative = pd.DataFrame.copy(similar_country, deep=True)
    similar_country_relative.iloc[0, 1:] = similar_country_relative.iloc[0, 1:].apply(
        lambda x: x / 5.806)  # Denmark
    similar_country_relative.iloc[1, 1:] = similar_country_relative.iloc[1, 1:].apply(
        lambda x: x / 5.518)  # Finland
    similar_country_relative.iloc[2, 1:] = similar_country_relative.iloc[2, 1:].apply(
        lambda x: x / 5.368)  # Norway
    similar_country_relative.iloc[3, 1:] = similar_country_relative.iloc[3, 1:].apply(
        lambda x: x / 10.23)  # Sweden


    # Change format from wide to long for plotting
    similar_country_long = pd.melt(
        similar_country_relative,
    ...
    similar_country_long.to_csv('../data/similar_country_relative_cases.csv')
    ...
    norway_indexes = tests[tests['Entity'] == 'Norway']['Total tests'].index.values
    tests.iloc[norway_indexes, 4] = tests.iloc[norway_indexes, 4].apply(
        lambda x: x/5.368)  # Norway
    finland_indexes = tests[tests['Entity'] == 'Finland']['Total tests'].index.values
    tests.iloc[finland_indexes, 4] = tests.iloc[finland_indexes, 4].apply(
        lambda x: x/5.518)  # Finland
    denmark_indexes = tests[tests['Entity'] == 'Denmark']['Total tests'].index.values
    tests.iloc[denmark_indexes, 4] = tests.iloc[denmark_indexes, 4].apply(
        lambda x: x /5.806)  # Denmark
    ...
    tests.to_csv('../data/similar_country_relative_tests.csv')
    ...
    fig.write_html("../figures/similar_country_relative.html")
